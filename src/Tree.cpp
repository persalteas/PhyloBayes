...
double Tree::ProposeTimeMove(double tuning) {
  ...
  double delta = tuning * (size * Random::Uniform() - size / 2.0);
  target->time = target->time * exp(delta);
  if (target->time < lower)
    target->time = lower + (lower - target->time);
  if (target->time > higher)
    target->time = higher - (target->time - higher);
  return delta;
}
...
Tree::Tree(int n, string *namelist) {
  ...
  for (int k = n; k > 1; k--) {
    ...
    // so that, after the merge, the first k-1 entries of the table correspond
    // to the k-1 remaining lineages
    if (maxi < k - 1) {
...
Node *Tree::ChooseInternalNodeAtRandom() {
  ...
  while (choose) {
    ...
    while ((k < nodelist.size()) &&
           ((nodelist[k]->isRoot()) || (nodelist[k]->isLeaf()))) {
      k++;
    }
    if (k == nodelist.size()) {
...
Node *Tree::ChooseSPRNodeAtRandom() {
  ...
  while (choose) {
    ...
    while ((k < nodelist.size()) &&
           ((nodelist[k]->isRoot()) || (nodelist[k]->up->isRoot()))) {
      k++;
    }
    if (k == nodelist.size()) {
      cerr << "error in choose node: overflow\n";
...
void Tree::RecursiveGetTargets(Node *node, vector<Node *> &list, double time,
                               Node *cousin) {

  if (!node->isRoot()) {
    if ((node->GetTime() < time) && (node->up->GetTime() > time) &&
        (node != cousin)) {
      list.push_back(node);
    }
  }
...
void Tree::ProposeSPRMove() {
  ...
  if (sizeafter != sizebefore) {
    ...
    cerr << targets.size() << '\n';
    cerr << newcousin->GetTime() << '\t' << father->GetTime() << '\t'
         << newgrandfather->GetTime() << '\n';
    exit(1);
  }
}
