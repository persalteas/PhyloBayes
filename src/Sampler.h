...
public:
  ...
  // MCMC
  // run a MCMC chain and regularly save state into files with specified name (+
  // extensions)
  void Run(string name) {
    ...
    while (count[4] < ITERMAX) {
      if (!(count[4] % 100))
        cout << "iteration " << count[4] << "/" << ITERMAX
             << " (acceptance rates: "
             << double(Naccepted[0]) / count[0] * 100.0 << "%, "
             << double(Naccepted[1]) / count[1] * 100.0 << "%, "
             << double(Naccepted[2]) / count[2] * 100.0 << "%)" << endl;
      count[4]++;
      Cycle();
      Trace(os);
      WriteTree(tos);
  ...
  // and for each site, should implement a recursive function across all nodes
  // of the tree
  void Pruning(int site, Node *node);

  // during the run, the MCMC sampler cycles over all moves:
  // the Cycle() function should call all types of moves, with good tuning
  // parameters
  void Cycle();
  ...
  // simple accessors
  Node *GetRoot() { return tree->GetRoot(); }
  ...
  void SampleRate() { rate = Random::sExpo(); }
