...
public:
  ...
  // construct a random tree, with n leaves, with names given by an array of
  // strings <namelist>
  ...
  // you can look at the GetSize() function for an example of a recursive
  // function
  double GetLength() { return RecursiveGetLength(root); }

  double RecursiveGetLength(Node *node) {
    if (!node->left) {
      return node->GetBranchLength();
    }
    return node->GetBranchLength() + RecursiveGetLength(node->left) +
           RecursiveGetLength(node->right);
  }
  ...
  int GetSize() { return RecursiveGetSize(root); }
  ...
  double ProposeTimeMove(double tuning);
...
private:
  ...
  void SetRoot(Node *inroot) { root = inroot; }

  // utilities for the SPR move
  Node *ChooseSPRNodeAtRandom();
  void RecursiveGetTargets(Node *node, vector<Node *> &list, double time,
                           Node *cousin);
